...
class ApiController extends BaseController
{
    ...
    public function get_rule()
    {
        $rule = get_admin_menu();
        $group = [];
        if (!empty(get_param('id'))) {
            $group = get_admin_role_info(get_param('id'))['rules'];
        }
    ...
    public function personal_submit()
    {
        if (request()->isAjax()) {
            $param = get_param();
            try {
                validate(AdminValidate::class)->scene('editPersonal')->check($param);
    ...
    public function password_submit()
    {
        if (request()->isAjax()) {
            $param = get_param();
            try {
                validate(AdminValidate::class)->scene('editpwd')->check($param);
            } catch (ValidateException $e) {
                // 验证失败 输出错误信息
                return $this->error($e->getError());
            }
            $admin = get_login_admin();
            if (create_password($param['old_pwd']) !== $admin['pwd']) {
                return $this->error('旧密码不正确!');
            }
            unset($param['username']);
            $param['salt'] = set_salt(20);
            $param['pwd'] = create_password($param['pwd']);
            Db::name('admin')->where([
                'id' => $admin['id'],
            ])->strict(false)->field(true)->update($param);
            $session_admin = get_config('app.session_admin');
    ...
    public function get_view_data()
    {
        ...
        return $this->success('', ['data_first' => hour_document($data_first), 'data_second' => hour_document($data_second), 'data_three'=>date_document($data_three)]);
    }
