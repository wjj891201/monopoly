...
{
    return Db::name('admin_role')->order('created_at asc')->select()->toArray();
}
...
{
    $cate = Db::name('article_cate')->order('created_at asc')->select()->toArray();
    return $cate;
}
...
function add_log($type, $itemId = '', $param = [])
{
    ...
    if ($type == 'login') {
        $login_admin = Db::name('admin')->where(array('id' => $itemId))->find();
    } else {
        $session_admin = get_config('app.session_admin');
